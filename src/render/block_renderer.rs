...
use std::ops::Deref;
use std::rc::Rc;
use crate::render::blocks_loader::{Block, BlocksLoader, BlockUsingError, UNKNOWN_BLOCK_ID};
...
pub(crate) fn render_block(world: &World, blocks_loader: &BlocksLoader, block_lid: u16, vertices: &mut Vec<Vertex>, indices: &mut Vec<i32>, pos: &Vec3ub) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match block.mesh.deref() {
        Cube(cube_mesh) => {
            ...
            for neighboring_face in neighboring_faces {
                if let Some(neighbor_block_lid) = get_neighbor_block(world, pos, &neighboring_face.0) {
        ...
        Custom(custom_mesh) => {
            if custom_mesh.faces.len() != 0 {
                ...
                for neighbor in NEIGHBORHOOD {
                    if let Some(neighbor_block_lid) = get_neighbor_block(world, pos, &neighbor) {
                        if let Some(neighbor_block) = blocks_loader.loaded_blocks.get(neighbor_block_lid as usize) {
                            if !neighbor_block.mesh.is_cube() {
                                rend = true;
                                break;
